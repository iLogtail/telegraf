...
.PHONY: build
build:
	CGO_ENABLED=0 go build -tags "$(BUILDTAGS)" -ldflags "$(LDFLAGS)" ./cmd/ilogtail_telegraf -o telegraf
...
.PHONY: go-install
go-install:
	go install -mod=mod -ldflags "-w -s $(LDFLAGS)" ./cmd/ilogtail_telegraf
...
$(buildbin):
	echo $(GOOS)
	@mkdir -pv $(dir $@)
	CGO_ENABLED=0 go build -o $(dir $@) -tags "$(BUILDTAGS)" -ldflags "$(LDFLAGS)" ./cmd/ilogtail_telegraf
